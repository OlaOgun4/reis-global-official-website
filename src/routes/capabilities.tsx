  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "REIS Global Capabilities" },
      {
        ...
        content:
          "Research-led engineering capability for secure, scalable and intelligent transformation.",
      },
...
const capabilityDomains = [
  ...
  },
  {
    ...
    summary:
      "Secure cloud environments and deployment models designed for scale, continuity and controlled change.",
  ...
  },
  {
    ...
    summary:
      "Research, service redesign, architecture and delivery combined into coherent transformation programmes.",
    ...
    <main className="reis-site min-h-screen overflow-x-hidden">
      <header className="reis-header fixed inset-x-0 top-0 z-50 border-b backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-6 lg:px-10">
          ...
          <nav className="hidden items-center gap-7 text-sm lg:flex">
            ...
            <a className="reis-nav-link text-white" href="/capabilities">Capabilities</a>
        ...
        <div className="relative mx-auto grid min-h-[640px] max-w-[1440px] items-center gap-12 px-6 py-20 lg:grid-cols-[1.05fr_.95fr] lg:px-10">
          <div className="z-10 max-w-3xl">
            ...
            <p className="reis-hero-copy mt-7 max-w-2xl text-lg leading-8 sm:text-xl">
              REIS combines research, architecture and engineering to design secure, scalable and intelligent digital platforms that enable meaningful transformation.
            </p>
          </div>
      ...
      <section className="reis-section py-24">
        <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
          <SectionHeading eyebrow="Capability domains" title="Eight disciplines. One integrated engineering model." text="Each capability can be applied independently, but the greatest value comes from combining them around a clear institutional mission." />
            ...
            {capabilityDomains.map(({ icon: Icon, title, summary, competencies, deliverables, evidence }, index) => (
              <article key={title} className="reis-panel rounded-2xl border p-7">
                <div className="flex items-start justify-between gap-5">
                  <div className="reis-small-icon grid h-12 w-12 shrink-0 place-items-center rounded-xl"><Icon size={24} /></div>
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  {competencies.map((item) => <span key={item} className="reis-tag rounded-full px-3 py-1.5 text-xs font-semibold">{item}</span>)}
                </div>
                <div className="mt-6 grid gap-4 border-t border-current/10 pt-5 sm:grid-cols-2">
                  <div><p className="text-xs font-bold uppercase tracking-[0.15em] opacity-45">Typical deliverables</p><p className="mt-2 text-sm leading-6 opacity-70">{deliverables}</p></div>
                  <div><p className="text-xs font-bold uppercase tracking-[0.15em] opacity-45">REIS evidence</p><p className="mt-2 text-sm leading-6 opacity-70">{evidence}</p></div>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="reis-research-section py-24">
        <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
          <SectionHeading eyebrow="Delivery methodology" title="From research to continuous improvement" text="A controlled lifecycle connects institutional purpose to architecture, implementation, assurance and measurable operational outcomes." />
      ...
      <section className="reis-section py-24">
        <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
          <SectionHeading eyebrow="Capability matrix" title="Engineering applied across priority sectors" text="The same governed capability model is adapted to the operating, regulatory and mission context of each sector." />
          <div className="mt-12 overflow-x-auto rounded-2xl border reis-panel">
            <table className="min-w-[880px] w-full text-left text-sm">
              <thead className="border-b border-current/10">
                <tr><th className="px-6 py-5">Capability</th>{["Government", "Enterprise", "Defence", "Agriculture", "Justice"].map((sector) => <th key={sector} className="px-5 py-5 text-center">{sector}</th>)}</tr>
              </thead>
              <tbody>
                {matrix.map(([name, ...checks]) => (
                  <tr key={name} className="border-b border-current/10 last:border-0">
                    ...
                    {checks.map((enabled, index) => <td key={`${name}-${index}`} className="px-5 py-4 text-center">{enabled && <span className="reis-check mx-auto grid h-7 w-7 place-items-center rounded-full"><Check size={14} /></span>}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="reis-dark-section py-24">
        <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
          <SectionHeading eyebrow="Engineering in practice" title="Capability demonstrated through real programmes" text="REIS work provides practical evidence of architecture, engineering, governance and transformation capability." dark />
  ...
  return (
    <div className="relative mx-auto h-[430px] w-full max-w-[560px]">
      <div className="absolute left-1/2 top-1/2 grid h-44 w-44 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-[#2caeff]/40 bg-[#082651]/75 shadow-[0_0_80px_rgba(0,120,255,.35)] backdrop-blur-xl">
        <div className="text-center"><div className="text-3xl font-bold text-white">REIS</div><div className="mt-2 text-xs uppercase tracking-[0.2em] text-[#5ed1ff]">Engineering Core</div></div>
      </div>
      ...
      {nodes.map(([Icon, label, position]) => (
        <div key={label} className={`absolute ${position} reis-panel flex items-center gap-3 rounded-xl border px-4 py-3`}>
          <div className="reis-small-icon grid h-9 w-9 place-items-center rounded-lg"><Icon size={18} /></div><span className="text-sm font-semibold">{label}</span>
        </div>
      ))}
    </div>
  );
